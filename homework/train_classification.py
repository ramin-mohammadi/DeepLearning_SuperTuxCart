...
from .datasets.classification_dataset import compute_accuracy


def train(
    exp_dir: str = "logs",
    model_name: str = "classifier",  # in models.py look at model_factory
    num_epoch: int = 30,
    lr: float = 1e-2,
    batch_size: int = 128,
    seed: int = 2024,
    **kwargs,
    ...
    for epoch in range(num_epoch):
        ...
        for img, label in train_data: # each iteration is a batch so img and label are a batch
            img, label = img.to(device), label.to(device)
            
            out = model(img)
            #print(out.shape) # batch_size, classes, width, height
           # print(torch.nn.functional.log_softmax(out, dim=1).shape)
